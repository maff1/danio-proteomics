################################################################################
# CBZ preprocess and run LIMMA ## v001 ## MAFF #################################
...
saveRDS(metaDat, "./RESULTS/metaData_cbz.rds")
...
samples <- data.frame(samples = rownames(expr), 
                      groups = gsub("\\.[0-9]$|\\.[0-9][A-Z]$|(?<=DMSO)[0-9]", "", 
                                    rownames(expr), perl = TRUE),
                      row.names = rownames(expr)
)
...
pdf(file = "./RESULTS/cbz_quantile.normalisation.pdf")
...
# TOO MUCH VARIATION IN THE MEAN INTENSITY AMONG CTR AND DMSO4 SAMPLES
# CTR.2       CTR.3       CTR.4      CTR.3A
# 7.4415708   0.5265584  14.7165392   0.5265584
# REMOVE SAMPLES CTR.3, CTR.3A AND DMSO2

print(colMeans(exprs(neset)))

filterSample <- colnames(neset)[neset@phenoData@data$samples %nin% c("CTR.3", "CTR.3A", "DMSO2")]
...
contrast =  makeContrasts(contrasts=c("CBZ.0.1-CTR","CBZ.100-CTR","CBZEP.0.1-CTR",
                                      "CBZEP.100-CTR", "DMSO-CTR", 
                                      "CBZ.0.1-DMSO",
                                      "CBZ.100-DMSO", "CBZEP.0.1-DMSO",
                                      "CBZEP.100-DMSO", "CTR-DMSO"), levels=design)
fit1 <- lmFit(exprs(neset), design)
fit2 <- contrasts.fit(fit1, contrasts = contrast)
fit3 <- eBayes(fit2, trend = TRUE)

lsContrast = list("CBZ.0.1-CTR","CBZ.100-CTR","CBZEP.0.1-CTR",
                  "CBZEP.100-CTR", "DMSO-CTR", 
                  "CBZ.0.1-DMSO",
                  "CBZ.100-DMSO", "CBZEP.0.1-DMSO",
                  "CBZEP.100-DMSO", "CTR-DMSO")
names(lsContrast) <- c("CBZ.0.1-CTR","CBZ.100-CTR","CBZEP.0.1-CTR",
                       "CBZEP.100-CTR", "DMSO-CTR", 
                       "CBZ.0.1-DMSO",
                       "CBZ.100-DMSO", "CBZEP.0.1-DMSO",
                       "CBZEP.100-DMSO", "CTR-DMSO")
...
if(!file.exists("./RESULTS/limma_DEP_cbz.xlsx")) {
  openxlsx::write.xlsx(lsResAnn, "./RESULTS/limma_DEP_cbz.xlsx", rowNames = TRUE)
}
# ---------------------------------------------------------------------------- #

out <- list(dep = lsResAnn, eset = neset)

return(out)
}

res <- f_clean_dep(
  pathDat = "Samples View Report for Danio rerio 210826 all samples.xls",
